...
Nous avons également utilisé pour le versionning **GIT** avec quatre repository distinct pour faciliter les évolutions de chaque partie sans que cela vienne interférer avec les autres.  
...
### Admin
Service permettant l'étude et la gestion des différentes données qui resortent du serious game.  
Il est accessible depuis le port **8081**.

### PgAdmin
Service qui va exposer l'outil de management de base de données pour PostgreSQL, PgAdmin.  
...
### Database
Service sur lequel la base de données PostgreSQL est exposée. Elle se situe sur le port **5432**.
...
**Attention** si vous êtes sous windows il est possible que vous ne puissiez exécuter le script car il est configuré en **LF** donc vous pouvez l'ouvrir dans un éditeur de code type vscode et venir modifier vous-même **CRLF => LF**. Pour l'exécuter :
```sh
./createEnv.sh
```
...
Normalement si vous avez suivi les étapes précédentes cet environnement est déjà quasiment prêt, il vous suffit maintenant de vous rendre dans le répertoire de votre API et d'éxecuter les lignes suivantes :
...
Après ça il ne vous restera plus qu'a lancer le programme pour ce faire rendez vous dans le dossier `docker-config` et exécutez la commande suivante :
```sh
...
./insert_base_data.sh
```
Cela va généré les données des tables de jeu, paramètres et token qui sont nécessaires au bon fonctionnement de l'application.
